...
  - output results to value of variable "output_dir"
...
def test_plot_peierls():
    ...
    plt.tight_layout()
    pdffile = os.path.join(output_dir, 'peierls_defmech.pdf')
...
def test_plot_peierls1():
    ...
    plt.tight_layout()
    pdffile = os.path.join(output_dir, 'peierls_defmech_1e-15.pdf')
    if os.path.isfile(pdffile):
      os.remove(pdffile)  # remove old file
    fig.savefig(pdffile,bbox_inches='tight')
    assert(os.path.isfile(pdffile))